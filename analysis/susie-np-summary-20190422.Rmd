...
$$
where $\hat{d}_{J-1,k}$ are mother wavelet coefficients at the finest level and $\hat{\sigma}$ is the estimated residual standard deviation. We first tried this method on [minimal examples](https://kaiqianzhang.github.io/susie-np/susie-residual-20190315.html) and observed fairly decent performance. We noticed that in the wavelet-based SuSiE, if we initialized from a SuSiE fit with a fixed residual variance estimated by MAD, and then fit SuSiE, we sometimes could avoid stucking with a local optimum problem and obtain a really nice fit. We performed [DSC simulations](https://kaiqianzhang.github.io/susie-np/susie-tf-MAD-20190416.html) to further evaluate MAD method. [FIXME: update DSC analysis for general change points problem]
