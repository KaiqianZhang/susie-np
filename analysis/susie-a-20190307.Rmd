...
I have a new R file having the function to update `ahat` and associated values stored in the fit `s`. 
...
The red line does not expand the parameter a and the blue line expands the parameter a. 
```{r}
...
Here is the problematic example we have before. The expanded parameter a at each iteration is almost 1. So the fit does not seem to change from the previous fit(i.e. the fit without expanding the parameter a). 
```{r}
f.beta5 = function(x){
  n = length(x)
  beta = numeric(n)
