...
## Simulate data and create X from decimated/non-decimated wavelets
...
```{r}
s.varbvsmix = varbvsmix(X=RW_t.decimated, Z=NULL, y=y, sa=sd^2)
```
...
Below we just use DWT-based matrix in the regression. The red line is varbvsmix fit, the blue line is varbvs fit, and the green line is SuSiE fit. We notice that SuSiE and varbvs are very similar. varbvs does not solve SuSiE's problem, which is having artifacts around jumps. And we observe that actually varbvsmix fit is shifted, which seems like the problem could be solved by introducing a scaling parameter (as discussed yesterday). But one thing I'm not sure is about my usage on varbvsmix. I set standard deviations for mixtures as 
...
and perform varbvsmix using both kinds of sd like

```
s.varbvsmix1 = varbvsmix(X=RW_t.decimated, Z=NULL, y=y, sa=sd^2)
s.varbvsmix2 = varbvsmix(X=RW_t.decimated, Z=NULL, y=y, sa=sd.grid^2)
```
